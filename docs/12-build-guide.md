...
**Everything in `dreamjob_server`, `dreamjob_app`, and `dreamjob_shared` gets typed by you.** Code you didn't write is code you can't defend when someone asks "walk me through how you did that" — which is exactly what [`resume-hiring-manager`](../skills/resume-hiring-manager/SKILL.md) exists to catch. A project you can't explain is worse than a smaller one you can.

Generated code from `build_runner` (freezed, json_serializable, drift) is fine — that's a compiler, not a shortcut. You still have to be able to explain what it generated and why.

## Where to start

Don't start at M1 and go in order. Start with the thinnest slice that produces a card on a screen, then widen it.

1. **`dreamjob_shared` first, `JobPosting` only.** One class, freezed, a JSON round-trip test. It's 40 lines and it forces the first real decision: what's nullable and why ([03](03-data-model.md) argues for `unknown` as a value — make sure you agree before you copy it).
...
3. **Store those postings in drift.** Now you have a schema, a migration, and a real question: what do you index?
...
5. **The matcher, as a pure function with a test file.** No UI. `dart test` printing scores is a demo.
...
- **Freeze the models before building the app.** Every `dreamjob_shared` change ripples through generated code on both sides. Cheap in week two, expensive in week eight.
...
- Explaining a concept (how drift migrations work, what a spring simulation is doing)
...
Scaffolding (`dart create`, project layout, a `pubspec.yaml`) is a grey area. Ask if you want it.
