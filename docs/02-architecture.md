...
- ~~Does attended browser mode make a desktop presence a requirement again?~~ **Resolved: no.** The phone is the only UI surface. The sidecar reports every run as screenshots and structured field data the app renders, and the submit token is minted from the phone ([14](14-browser-agent.md) §5). A halt that genuinely needs the live browser waits for the user to reach the machine, or falls back to a manual draft.
